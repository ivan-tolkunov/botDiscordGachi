    ...
    async def play_sound(self, content, typ):
        if self.voice != None:
            if typ == "sound":
                ...
                self.voice.play(discord.FFmpegPCMAudio(executable="ffmpeg/bin/ffmpeg.exe", source=f"{MAIN_PATH}{content}"))
                while self.voice.is_playing():
                    await asyncio.sleep(.1)
            elif typ == "music":
                self.voice.play(discord.FFmpegPCMAudio(executable="ffmpeg/bin/ffmpeg.exe", source=f"{content}"))
                while self.voice.is_playing():
                    await asyncio.sleep(.1)
